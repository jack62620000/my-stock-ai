...
def get_buffett_pro_analysis(d, code, api_key):
    try:
        ...
        2. 💎【巴菲特的內在價值審查】：
           {d['name']}從毛利與財務穩健度看，是否有寬廣護城河？護城河有哪些？全球市佔率預估是多少？在目前通膨環境下是否具備「定價權」？

        3. 📉【股價走勢與動能判斷】：
           {d['name']}目前的 K線、KD、MACD、RSI 透露什麼買賣訊號？目前股價是反應基本面價值，還是處於市場情緒的過度波動？

        4. 🎯【法人目標價與達成時間預估】：
           {d['name']}分析法人平均目標價 {d['target_mean'] if d['target_mean'] else 'N/A'} 的合理性。預估股價達到合理價的「時間點」，並說明理由。

        5. 📈【終極投資策略建議】：
           對於{d['name']}給出具體的「長線持有」或「短線避險」建議。
           - 長線進場價：
           - 短線進場價：
           - 停損點建議：
        """
