    ...
    if d:
        ...
        with st.container(border=True):
            ...
            with f1:
                ...
                st.write(f"營業利益率: {round(d['op']*100, 2)}%")
            with f2:
                st.write("**【 進階風險 】**")
                st.write(f"自由現金流: {round(d['fcf'], 1)} 億")
                st.write(f"負債比率: {round(d['debt']*100, 1)}%")
                st.write(f"速動比率: {round(d['quick']*100, 1)}%")
            with f3:
                st.write("**【 成長過濾 】**")
            ...
            with f4:
                ...
                elif d['pos_52'] > 0.8: st.error("🟠 過熱警戒")
                else: st.info("⏳ 中性觀望")
        ...
        with st.container(border=True):
            ...
            with t1:
                ...
                st.write(f"MA60 (季線): {round(latest['MA60'], 1)}")
            ...
            with t2:
                ...
                st.write(f"量能噴發比: {round(v_ratio, 2)}x")
                st.write(f"RSI(14): {round(latest['RSI'], 1)}")
            with t3:
                st.write("**【 動能指標 】**")
                k, dv = d['stoch'].iloc[-1, 0], d['stoch'].iloc[-1, 1]
                st.write(f"KD值: K{round(k,1)} / D{round(dv,1)}")
                st.write(f"KD狀態: {'🔥金叉' if k>dv else '❄️死叉'}")
                m_hist = d['macd'].iloc[-1, 2]
                st.write(f"MACD柱狀體: {round(m_hist, 2)}")
            with t4:
                st.write("**【 籌碼與區間 】**")
                st.write(f"布林上軌: {round(d['bol'].iloc[-1, 2], 1)}")
                st.write(f"布林下軌: {round(d['bol'].iloc[-1, 0], 1)}")
