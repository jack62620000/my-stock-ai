...
#  ========== 一、基本面（公司賺不賺錢）==========
        ...
        with st.container(border=True):
            ...
            f2.metric("負債/股東權益", f"{debt_to_equity:.1f}% ({dte_text})",help=f"負債/股東權益：{debt_to_equity:.1f}% ({dte_text})\n衡量公司使用借貸槓桿的程度。")
            # 流動比率
            ...
            f3.metric("流動比率", f"{current_ratio:.2f} ({current_text})",help=f"流動比率：{current_ratio:.2f} ({current_text})\n衡量公司短期償債能力，一般以 1.5～2 倍為佳。")
            # 速動比率
            ...
            f4.metric("速動比率", f"{quick_ratio:.2f} ({quick_text})",help=f"速動比率：{quick_ratio:.2f} ({quick_text})\n扣除存貨後的短期償債能力指標，愈高愈好。")
            # 存貨佔資產比
            ...
            if inv_asset_ratio > 0.5:
                ...
                inv_text = ":red[偏低⭐️]"
            f5.metric("存貨佔資產比", f"{inv_asset_ratio:.1%} ({inv_text})",help=f"存貨佔資產比：{inv_asset_ratio:.1%} ({inv_text})\n存貨過高可能有跌價與庫存風險。")
            # 現金佔資產比
            ...
            f6.metric("現金佔資產比", f"{cash_asset_ratio:.1%} ({cash_text})",help=f"現金佔資產比：{cash_asset_ratio:.1%} ({cash_text})\n愈高代表公司現金儲備愈充裕。")
            # 非流動負債占負債比
            ...
            if ncd_liabilities_ratio >0.8:
                ...
                ncd_text = ":red[偏低⭐️]"
            f7.metric("非流動負債占負債比", f"{ncd_liabilities_ratio:.1%} ({ncd_text})",help=f"非流動負債占負債比：{ncd_liabilities_ratio:.1%} ({ncd_text})\n長期負債佔比愈高，財務結構愈偏長期化，但也可能增加利息支出壓力。")
            # 現金流品質
            st.subheader("現金流品質")
            ...
            if cashflow_profit_ratio > 1:
                ...
                cfp_text =":green[偏低⚠️]"
            ca3.metric("現金流/淨利", f"{cashflow_profit_ratio:.2f} ({cfp_text})",help=f"現金流/淨利：{cashflow_profit_ratio:.2f} ({cfp_text})\n>1 代表現金流比淨利佳，公司獲利品質較好。")
            # FCF/營收
            ...
            ca4.metric("FCF/營收", f"{fcf_revenue_ratio:.1%} ({fcf_rev_text})",help=f"FCF/營收：{fcf_revenue_ratio:.1%} ({fcf_rev_text})\n自由現金流佔營收的比例，愈高代表現金生成力愈強。")
            # FCF/股價
            ...
            ca5.metric("FCF/股價", f"{fcf_price_ratio:.1%} ({fcf_price_text})",help=f"FCF/股價：{fcf_price_ratio:.1%} ({fcf_price_text})\n每單位股價背後有多少自由現金流支撐。")
            # FCF 成長率
            ...
            ca6.metric("FCF 成長率", f"{fcf_growth:.1f}% ({fcf_growth_text})",help=f"FCF 成長率：{fcf_growth:.1f}% ({fcf_growth_text})\n自由現金流的年成長率，代表現金生成力是否在提升。")
            # 資本支出／營業現金流
            ...
            ca7.metric("資本支出/營業現金流", f"{capex_to_cashflow:.2f} ({capex_cf_text})",help=f"資本支出/營業現金流：{capex_to_cashflow:.2f} ({capex_cf_text})\n>1 代表資本支出比營業現金流還多，可能有現金流壓力。")
            # 估值水準
            st.subheader("估值水準")
            ...
            if pb > 3:
                ...
                pb_text = ":red[偏低估⭐️]"
            v2.metric("股價淨值比(P/B)", f"{pb:.1f}x ({pb_text})",help=f"股價淨值比(P/B)：{pb:.1f}x ({pb_text})\n偏高代表可能高估，偏低代表可能低估，但與產業特性有關。")
            # PEG
            peg = d.get("peg", 0)
            if peg > 1.5:
                ...
                peg_text = ":red[偏低估⭐️]"
            v3.metric("PEG", f"{peg:.1f} ({peg_text})",help=f"PEG：{peg:.1f} ({peg_text})\nPEG 用成長率調整本益比，愈接近 1 愈合理。")
            # 股利殖利率
            ...
            if div_yield > 5:
                ...
                div_text =":green[偏低⚠️]"
            v4.metric("股利殖利率", f"{div_yield:.1f}% ({div_text})",help=f"股利殖利率：{div_yield:.1f}% ({div_text})\n偏高代表現金回報較高，但要注意是否可持續。")
            # 盈餘配發率
            ...
            v5.metric("盈餘配發率", f"{payout_ratio:.1f}% ({payout_text})",help=f"盈餘配發率：{payout_ratio:.1f}% ({payout_text})\n偏高代表多數盈餘用於配股，偏低則代表多數盈餘用於再投資。")
            # 現金股利報酬率
            ...
            if cash_dividend_yield > 5:
                ...
                cd_text =":green[偏低⚠️]"
            v6.metric("現金股利報酬率", f"{cash_dividend_yield:.1f}% ({cd_text})",help=f"現金股利報酬率：{cash_dividend_yield:.1f}% ({cd_text})\n多少現金股利相對於股價的比例。")
            # 帳面價值成長率
            ...
            if book_value_growth > 8:
                ...
                bv_text =":green[偏低⚠️]"
            v7.metric("帳面價值成長率", f"{book_value_growth:.1f}% ({bv_text})",help=f"帳面價值成長率：{book_value_growth:.1f}% ({bv_text})\n代表公司帳面資產與淨值的成長速度。")
#  ========== 二、技術面（趨勢、動能、波動、量價）==========
        ...
        if not df.empty:
            ...
            with st.container(border=True):
                ...
                if price > ma5:
                    ...
                    ma5_text = ":green[空頭]"
                t1.metric("MA5", f"{ma5:.1f} ({ma5_text})",help=f"MA5：{ma5:.1f} ({ma5_text})\n5日均線，股價在均線上方為多頭。")
                # MA20
                ma20 = latest.get("ma20", price)
                if price > ma20:
                    ma20_text = ":red[多頭]"
                    ...
                    ma20_text = ":green[空頭]"
                t1.metric("MA20", f"{ma20:.1f} ({ma20_text})",help=f"MA20：{ma20:.1f} ({ma20_text})\n20日均線，中期趨勢判斷。")
                # MA60
                ma60 = latest.get("ma60", price)
                if price > ma60:
                    ma60_text = ":red[多頭]"
                    ...
                    ma60_text = ":green[空頭]"
                t1.metric("MA60", f"{ma60:.1f} ({ma60_text})",help=f"MA60：{ma60:.1f} ({ma60_text})\n60日均線，長期趨勢判斷。")
                bias = d.get("bias", 0)
                if bias > 10:
                    ...
                    bias_text = ":orange[合理🟡]"
                t1.metric("乖離率", f"{bias:.1f}% ({bias_text})",help=f"乖離率：{bias:.1f}% ({bias_text})\n股價偏離20日均線的百分比，±10%內為合理範圍。")
                # 動能與強度
                t2.subheader("動能與強度")
                rsi = d.get("rsi", 50)
                if rsi > 70:
                ...
                if macd_line > macd_signal:
                    ...
                    macd_text = ":green[空頭]"
                t2.metric("MACD 本體", f"{macd_line:+.2f} ({macd_text})",help=f"MACD 本體：{macd_line:+.2f} ({macd_text})MACD線高於信號線為多頭訊號。")
                if macd_signal > 0:
                    signal_text = ":red[多頭]"
                else:
                    signal_text = ":green[空頭]"
                t2.metric("MACD 信號線", f"{macd_signal:+.2f} ({signal_text})",help=f"MACD 信號線：{macd_signal:+.2f} ({signal_text})信號線>0為多頭趨勢。")
                # 波動與區間
                t3.subheader("波動與區間")
                ...
                t3.metric("布林上軌", f"{bb_upper:.1f} ({bb_upper_text})",help=f"布林上軌：{bb_upper:.1f} ({bb_upper_text})\n股價突破上軌代表強勢。")
                t3.metric("布林中軌", f"{bb_mid:.1f}",help=f"布林中軌 {bb_mid:.1f} - 20日移動平均線")
                if price < bb_lower:
                ...
                t3.metric("52週高價", f"{high_52:.1f} ({high_text})",help=f"52週高價：{high_52:.1f} ({high_text})\n過去52週最高價。")
                if price < low_52:
                    low_text = ":red[52週新低(多頭弱、但也偏便宜)]"
                else:
                    low_text = ":orange[未新低🟡]"
                t3.metric("52週低價", f"{low_52:.1f} ({low_text})",help=f"52週低價：{low_52:.1f} ({low_text})\n過去52週最低價。")
                ...
                t3.metric("標準差(20日)", f"{std_20:.2f} ({std_text})",help=f"標準差(20日)：{std_20:.2f} ({std_text})\n衡量股價波動程度。")
                atr = d.get("atr", 0)
                if atr > 3:
                ...
                t3.metric("ATR(14日)", f"{atr:.2f} ({atr_text})",help=f"ATR(14日)：{atr:.2f} ({atr_text})\n平均真實波幅，衡量每日波動範圍。")
                # 成交量與量價關係
