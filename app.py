...
if code_input:
    ...
    if d:
        ...
        # --- 第三部分：Gemini AI (修正 NameError 與連線) ---
        st.divider()
        st.subheader("🤖 Gemini AI 專家點評")
        
        # 從 Secrets 獲取新申請的 Key
        ...
        if api_key:
            try:
                ...
                # 簡單測試 Prompt
                prompt = f"妳是專業台股分析師。請針對 {d['name']}({code_input}) 分析：價格 {d['p']}，ROE {round(d['roe']*100,1)}%。請給出 40 字內的投資建議。"
                ...
                if response and response.text:
                    ...
                    st.warning("AI 連結成功但未回傳文字，請重新整理。")
                    
            except Exception as error:
                # 這裡正確處理 e (改名為 error 避免混淆)
                st.error(f"⚠️ AI 連線細節：{str(error)}")
        else:
